class CloudVolumeController < ApplicationController
  ...
  def attach
    ...
    @edit[:vm_choices] = {}
    @volume = find_by_id_filtered(CloudVolume, params[:id])
    @volume.cloud_tenant.vms.each { |vm| @edit[:vm_choices][vm.name] = vm.id }

    @in_a_form = true
    drop_breadcrumb(
      :name => _("Attach %{model} \"%{name}\"") % {:model => ui_lookup(:table => 'cloud_volume'), :name => @volume.name},
  ...
  def detach
    ...
    attached_vms.each { |vm| @edit[:vm_choices][vm.name] = vm.id }

    @in_a_form = true
    drop_breadcrumb(
      :name => _("Detach %{model} \"%{name}\"") % {:model => ui_lookup(:table => 'cloud_volume'), :name => @volume.name},
  ...
  def attach_volume
    ...
    when "cancel"
      ...
      @volume = find_by_id_filtered(CloudVolume, params[:id])
      flash = _("Attaching %{model} \"%{name}\" was cancelled by the user") % {
    ...
    when "attach"
      ...
      if valid_attach
        begin
          @volume.attach_volume(vm.ems_ref, options['device_path'])
  ...
  def detach_volume
    ...
    when "cancel"
      ...
      @breadcrumbs.pop if @breadcrumbs
      @volume = find_by_id_filtered(CloudVolume, params[:id])
      flash = _("Detaching %{model} \"%{name}\" was cancelled by the user") % {
        :model => ui_lookup(:table => 'cloud_volume'),
        :name  => @volume.name
      }
      render :update do |page|
    ...
    when "detach"
      ...
      if valid_detach
        begin
          @volume.detach_volume(vm.ems_ref)
  ...
  def update
    ...
    when "reset"
      ...
      session[:flash_msgs] = @flash_array.dup
      render :update do |page|
        page.redirect_to :action => 'edit', :id => @volume.id
      end
  ...
  def construct_edit
    @edit ||= {}
    @edit[:current] = {:display_name => @volume.name, :size => @volume.size}
    @edit[:new] = {}
    @edit[:new][:display_name] = params[:name] if params[:name]
    @edit[:new][:size] = params[:size].to_i if params[:size]
    @edit[:new][:cloud_tenant_id] = params[:cloud_tenant_id] if params[:cloud_tenant_id]
    @edit[:new][:vm_id] = params[:vm_id] if params[:vm_id]
    @edit[:new][:device_path] = params[:device_path] if params[:device_path]
  end
